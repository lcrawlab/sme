...
`mme()` function and how to adjust parameters to optimize memory consumption
...
of the C++ objects stored in memory during the `mme()` function call, though it
...
To manage large datasets more efficiently, `mme()` reads the genotype data in
...
The `mme()` function uses a stochastic trace estimator to approximate the trace
...
<img src="../reference/figures/mme-shared-rvs.png"  width="100%"
alt="Multimodal Marginal Epistasis test (MME) schematic pseudo-random vectors"/>
...
marginal epistasis (MME) test approximates the traces using random vectors $z$.
...
## A Note on the Runtime of MME

Despite the computational efficiency of MME, genome-wide testing requires 
...
<img src="../reference/figures/mme-runtime.png"
width="100%" alt="Multimodal Marginal Epistasis test (MME) runtime" />
**Figure 2. ** MME has improved power to detect marginal epistasis and runs
